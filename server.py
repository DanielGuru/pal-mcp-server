...
async def execute_tool(name: str, arguments: dict[str, Any]) -> list[TextContent]:
    ...
    # Open a graph run for this dispatch. run_context auto-derives the parent
    ...
    # NOTE: depth-based dispatch tracking was removed (commit fixing the
    # audit's #1 finding). Size-check bypass is now provenance-based via
    # tools.shared.base_tool.mark_internal_payload(), set by code paths
    # that build internal payloads (multiaudit, panel debate/judge, clink
    # OAuth fallback). User-originated start_task / panel calls no longer
    # bypass the MCP-transport size check.
    ...
    # by the run_context block below.
    edge_kind = arguments.pop("_graph_edge_kind", "spawn")
    graph_cost_tier = arguments.pop("_graph_cost_tier", None)
    graph_label = arguments.pop("_graph_label", None)
...
async def main():
    ...
    cost_routing = (
        ...
        "string (gpt-5.5, gemini-3.1-pro-preview, grok-4.3, claude-opus-4-8, claude-sonnet-4-6, etc.), or "
        "asks for grok in any form, `chat`/`consensus` is the path. Anthropic is now a first-party "
        "provider — `chat(model='claude-opus-4-8')` and friends work directly via the Anthropic API "
    ...
    async_routing = (
        ...
        "**FIRE AND FORGET IS THE DEFAULT.** Once you've dispatched a long task via start_task (or "
        "called multiaudit/bugfind which dispatch internally), DO NOT silently poll task_status / "
        "task_result. Panel installs a Stop/UserPromptSubmit hook that delivers the verdict via "
        "<system-reminder> automatically when the run finishes — your job is to (1) hand the user "
        "the live web viewer URL immediately, (2) end your turn, (3) wake up via the system-reminder "
        "with the digest already inline. Polling silently in a tool-call loop wastes turns and shows "
        "the user nothing — they see neither the dispatch confirmation nor the eventual verdict "
        "until your loop exits. If the user EXPLICITLY asks for a status check (\"how's the panel "
        "doing?\"), then ONE call to task_status with wait_seconds<=10 is fine — surface the "
        "progress event count + elapsed seconds back to the user in plain text, then end the turn. "
        "The wait_seconds field is hard-capped at the PANEL_TASK_WAIT_CAP_S env (default 30). "
    ...
    panel_routing = (
        ...
        "mode: each model sees others' answers and revises)\n"
        "  - 'consensus' / 'agree on' / 'do they agree' → panel with "
        "judge='codex' (or 'gemini'); consider debate_rounds=1 if contested\n"
        "**Async pattern — read carefully, this is where LLMs trip up:**\n"
        "  - `multiaudit` and `bugfind` are ALREADY async by design — they "
        "dispatch the panel via start_task INTERNALLY and return immediately "
        "with a task_id pointing at the inner panel run. **Call them DIRECTLY**, "
        "do NOT wrap in start_task. The server now rejects double-wrapping "
        "with a clear error, but the right shape is `multiaudit(...)` / "
        "`bugfind(...)` with no start_task in front. Poll the task_id they "
        "return; that's the inner panel run.\n"
        "  - `panel` and `ask_panel` are SYNCHRONOUS fan-outs (they await all "
        "panelist responses before returning), so they can run for minutes. "
        "**Wrap these in start_task** so the conversation isn't blocked: "
        "`start_task(tool='ask_panel', arguments={...})`.\n"
        "For free OAuth runs use codex+gemini+claude panelists with codex as "
        "judge — three frontier families at zero cost. Add grok-4.3 only when "
        "the user asks for paid model diversity or names it explicitly. "
