...
This guide covers deploying Panel MCP Server using Docker and Docker Compose for production environments.
...
# View health status
docker inspect panel-mcp-server --format='{{.State.Health.Status}}'
```
...
- **Config**: `panel-mcp-config:/app/conf` - Configuration persistence
- **Time sync**: `/etc/localtime:/etc/localtime:ro` - Host timezone sync

**Note:** The `panel-mcp-config` is a named Docker volume that persists configuration data between container restarts. All data placed in `/app/conf` inside the container is preserved thanks to this persistent volume. This applies to both `docker-compose run` and `docker-compose up` commands.
...
```bash
# Check logs
docker-compose logs panel-mcp

# Manual health check
docker exec panel-mcp-server python /usr/local/bin/healthcheck.py
```
...
docker exec panel-mcp-server env | grep -E "(GEMINI|OPENAI|XAI)"
```
...
docker run --rm -v panel-mcp-config:/data -v $(pwd):/backup alpine tar czf /backup/config-backup.tar.gz -C /data .

# Restore configuration
docker run --rm -v panel-mcp-config:/data -v $(pwd):/backup alpine tar xzf /backup/config-backup.tar.gz -C /data
```
  ...
  "mcpServers": {
    "panel-mcp": {
      ...
      "args": [
        ...
        "-i",
        "--env-file",
        "C:/path/to/panel-mcp-server/.env",
        "-v",
        "C:/path/to/panel-mcp-server/logs:/app/logs",
        "panel-mcp-server:latest"
      ]
    }
  }
...
```json
{
  "mcpServers": {
    "panel-mcp": {
      ...
      "args": [
        "-f", "/absolute/path/to/panel-mcp-server/docker-compose.yml",
        "run", "--rm", "panel-mcp"
      ]
    }
  }
...
**Important notes:**
- Replace `/absolute/path/to/panel-mcp-server` with the actual path to your project.
- Always use forward slashes `/` for Docker volumes, even on Windows.
- Ensure the `.env` file exists and contains your API keys.
- **Persistent volumes**: Docker Compose options (Options 2) automatically use the `panel-mcp-config` named volume for persistent configuration storage.
...
**Troubleshooting:**
- If Option 1 fails: check that the Docker image exists (`docker images panel-mcp-server`).
...
When upgrading, configuration is preserved in the named volume `panel-mcp-config`.
