...
This guide covers setting up multiple AI model providers including OpenRouter, custom API endpoints, and local model servers. The Panel MCP server supports a unified configuration for all these providers through a single model registry.
...
Panel ships multiple registries:
...
The Panel MCP server runs natively, so you can use standard localhost URLs to connect to local models:
...
"Use pro via panel to analyze this"    # → google/gemini-2.5-pro
"Use gpt4o via panel to analyze this"  # → openai/gpt-4o
"Use mistral via panel to optimize"    # → mistral/mistral-large
...
```
# OpenRouter models:
"Use anthropic/claude-opus-4 via panel for deep analysis"
"Use openai/gpt-4o via panel to debug this"
"Use deepseek/deepseek-coder via panel to generate code"

# Local/custom models:
"Use llama3.2 via panel to review this"
"Use meta-llama/Llama-2-7b-chat-hf via panel to analyze"
```
