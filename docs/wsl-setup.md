...
This guide provides detailed instructions for setting up Panel MCP Server on Windows using WSL.
...
3. **Verify Claude Code can find the MCP server**:
   ...
   # You should see 'panel' listed in the output
...
- Always use the full WSL path for MCP configuration (e.g., `/home/YourName/panel-mcp-server/`)
...
cat ~/.claude.json | grep -A 10 "panel"

# The configuration should show the correct WSL path to the Python executable
# Example: "/home/YourName/panel-mcp-server/.pal_venv/bin/python"
```

### Performance Tip

For best performance, keep your panel-mcp-server directory in the WSL filesystem (e.g., `~/panel-mcp-server`) rather than in the Windows filesystem (`/mnt/c/...`).