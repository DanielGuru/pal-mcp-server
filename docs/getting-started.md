# Getting Started with Panel MCP Server

This guide walks you through setting up the Panel MCP Server from scratch, including installation, configuration, and first usage.
...
```json
{
  "mcpServers": {
    "panel": {
      ...
      "args": [
        "-c", 
        "for p in $(which uvx 2>/dev/null) $HOME/.local/bin/uvx /opt/homebrew/bin/uvx /usr/local/bin/uvx uvx; do [ -x \"$p\" ] && exec \"$p\" --from git+https://github.com/BeehiveInnovations/panel-mcp-server.git panel-mcp-server; done; echo 'uvx not found' >&2; exit 1"
      ],
      "env": {
...
Enable Codex's built-in web-search tool so Panel's `apilookup` instructions can execute successfully:
...
```json
{
  ...
  "mcp": {
    "panel": {
      "type": "local",
      "command": [
        "/path/to/panel-mcp-server/.pal_venv/bin/python",
        "/path/to/panel-mcp-server/server.py"
      ],
      "cwd": "/path/to/panel-mcp-server",
      "enabled": true,
      "environment": {
...
Panel works in GUI IDEs that speak MCP. The configuration mirrors the CLI examples above—point the client at the `uvx` launcher and set any required environment variables.
...
2. Click **Add MCP Server** and supply the following values:
   - Command: `sh`
   - Args: `-c` and `for p in $(which uvx 2>/dev/null) $HOME/.local/bin/uvx /opt/homebrew/bin/uvx /usr/local/bin/uvx uvx; do [ -x "$p" ] && exec "$p" --from git+https://github.com/BeehiveInnovations/panel-mcp-server.git panel-mcp-server; done; echo 'uvx not found' >&2; exit 1`
   - Environment (example):
     - `PATH=/usr/local/bin:/usr/bin:/bin:/opt/homebrew/bin:~/.local/bin`
     - `GEMINI_API_KEY=your_api_key_here`
...
👉 Pro tip: If you prefer a one-line command, replace the long loop with `uvx --from git+https://github.com/BeehiveInnovations/panel-mcp-server.git panel-mcp-server`—just make sure `uvx` is on your PATH for every client.
...
Some MCP clients default to short timeouts and can disconnect from Panel during long tool runs. Configure each client with a generous ceiling (we recommend at least five minutes); the Panel setup script now writes a 20-minute tool timeout for Codex so upstream providers contacted by the server have time to respond.
...
You can scope this block at the top level of `settings.json` (applies to every session) or under a specific `mcpServers.<name>.env` entry if you only want it for Panel (the server name may still be `panel` while configurations catch up). The values are in milliseconds. Note: Claude’s SSE transport still enforces an internal ceiling of roughly five minutes; long-running HTTP/SSE servers may need retries until Anthropic ships their fix.
...
3. Try: `"Use panel to list available models"`

### For Claude Code CLI:
1. Exit any existing Claude session
2. Run `claude` from your project directory  
3. Try: `"Use panel to chat about Python best practices"`

### For Gemini CLI:
**Note**: While Panel MCP connects to Gemini CLI, tool invocation isn't working correctly yet. See [Gemini CLI Setup](gemini-setup.md) for updates.

### For Qwen Code CLI:
1. Restart the Qwen Code CLI if it's running (`qwen exit`).
2. Run `qwen mcp list --scope user` and confirm `panel` shows `CONNECTED`.
3. Try: `"/mcp"` to inspect available tools or `"Use panel to analyze this repo"`.

### For OpenCode CLI:
1. Restart OpenCode (or run `OpenCode: Reload Config`).
2. Open **Settings › Tools › MCP** and confirm `panel` is enabled.
3. Start a new chat and try: `"Use panel to list available models"`.
...
### Test Commands:
```
"Use panel to list available models"
"Chat with panel about the best approach for API design"
"Use panel thinkdeep with gemini pro about scaling strategies"  
...
## Step 5: Start Using Panel

### Basic Usage Patterns:

**Let Claude pick the model:**
```
"Use panel to analyze this code for security issues"
"Debug this race condition with panel"
"Plan the database migration with panel"
```

**Specify the model:**
```  
"Use panel with gemini pro to review this complex algorithm"
"Debug with o3 using panel for logical analysis"
"Get flash to quickly format this code via panel"
```

**Multi-model workflows:**
```
"Use panel to get consensus from pro and o3 on this architecture"
...
### "panel not found" or "command not found"
...
**"Model not available":**
- Run `"Use panel to list available models"` to see what's configured
