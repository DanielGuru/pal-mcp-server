...
run Panel's planner with two separate ideas. Then when it's done, use Panel's `consensus` tool to pass the entire
...
```
Use panel's planner and show me how to add real-time notifications to our mobile app
```
...
```
Create a plan using panel for migrating our React app from JavaScript to TypeScript
```

```
Develop a plan using panel for implementing CI/CD pipelines across our development teams
```
...
Like all other tools in Panel, you can `continue` with a new plan using the output from a previous plan by simply saying

```
Continue with panel's consensus tool and find out what o3:for and flash:against think of the plan 
```
