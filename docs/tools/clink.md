...
- **Cross-tool collaboration**: Combine with other Panel tools like `planner` → `clink` → `codereview`
...
2. **Background execution** - Panel spawns the configured CLI (e.g., `gemini --output-format json`)
...
6. **Continuation support** - Future tools and models can reference Gemini's findings via [continuation support](../context-revival.md) within Panel.
...
- **Combine with Panel tools**: Chain `clink` with `planner`, `codereview`, `debug` for powerful workflows
...
- **Use `chat`** for: Direct model-to-model conversations within Panel
- **Use `planner`** for: Panel's native planning workflows with step validation
- **Use `codereview`** for: Panel's structured code review with severity levels
...
- [Planner Tool](planner.md) - Panel's native planning workflows
