...
1. **Streaming v2 — per-token for direct-API providers.** Anthropic streams unconditionally (`providers/anthropic.py` uses `client.messages.stream` and pumps text-chunk events into the graph). OpenAI / xAI also stream by default; integration cassette tests opt out via `PAL_OPENAI_STREAM=0` because the recorded fixtures hash a non-streaming request body — re-record cassettes to lift that constraint. Still TODO: Gemini (different SDK shape — `client.models.generate_content_stream`) and the OpenAI Responses endpoint (gpt-5.1-codex / o3-pro family — separate streaming API).
