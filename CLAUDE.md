...
- **Execution graph is per-repo by default.** Default DB path is `<cwd>/.pal/execution_graph.db` so each project Claude Code opens has its own isolated debate history. `.pal/` is gitignored. Override with `PAL_GRAPH_DB=<path>` for a shared/global view. The web viewer attached to a PAL instance only ever sees that instance's DB.
...
- **Web viewer is lazy-started and per-session.** Does NOT boot at MCP server startup; the first `execute_tool` call lazy-starts it. Means: open Claude Code without using PAL → no tab. First PAL tool call → tab pops, viewer runs for the rest of the process. Process exit → viewer dies with it. Bound to `127.0.0.1` by default. Boot failures log and continue. Disabled via `PAL_WEB_DISABLE`.
...
| `PAL_GRAPH_DB` | `<cwd>/.pal/execution_graph.db` | Path to SQLite execution graph. **Per-repo by default** so each project has its own debate history; the web viewer attached to that PAL instance only sees this repo's runs. Set to an absolute path to pin globally (e.g. legacy `~/.pal/execution_graph.db`); `""` disables. |
...
1. **Streaming v2 — per-token for direct-API providers.** Today's streaming-v1 surfaces clink's `describe_event` progress (file_read / tool_use / text_chunk) live in the viewer because clink's parser already emits per-event. Direct-API providers (chat → openai/xai/gemini SDKs) do `.create()` calls inside `asyncio.to_thread` and produce ZERO mid-flight events; the viewer just shows "RUNNING" until the API returns. To get per-token streaming for grok / gpt-5.5: switch `openai_compatible.py` to `stream=True`, pump deltas through `emit_progress` → graph → viewer. Requires touching providers/openai_compatible.py response shape.
...
3. **TaskManager → execution graph migration.** `TaskManager._tasks` is still in-memory. Migration means `start_task` / `task_status` / `task_result` / `cancel_task` survive PAL restart. Pair with stale-running handling because `_gc()` only evicts terminal records.
...
8. **Cancel-aware semaphore release.** Cancelling a panel mid-flight leaves the worker thread running its sync SDK call (asyncio limitation). The SDK timeout bounds this. Future: track in-flight thread count and refuse new tasks when exhausted.
9. **Web viewer auth.** Bound to 127.0.0.1 by default. If the user opts into `PAL_WEB_HOST=0.0.0.0`, today there's no auth — anyone on the network can read full args/results from the graph. Add a token in env or refuse non-localhost binds without one.
