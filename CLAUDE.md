...
A Model Context Protocol server that lets one AI agent (typically Claude Code) consult, orchestrate, and debate multiple other models. 30 MCP tools in six families:
...
5. **PR-shaped audit + bug-shaped investigation** — `multiaudit` reads `git diff`, packages it with intent context, and fires `start_task('panel', ...)` with `[codex, gemini, claude-sonnet-5 (label "sonnet"), claude-opus-4-8 (label "opus")]`. Both Anthropic slots are OAuth-first via the user's Claude subscription (sonnet routes to clink `claude`, opus routes to clink `claude_opus` with `--model opus`), with paid-API fallback on quota. `bugfind` (sister tool, same shape) takes a `bug_description` plus auto-collected context (recent commits, error log tail, optionally attached files) and fires the same 4-way panel with a bug-investigation rubric (REPRO / ROOT CAUSE / MINIMAL FIX / REGRESSION TEST / BLAST RADIUS / WHAT YOU MISSED). `host` is intentionally excluded from defaults because Claude Code doesn't advertise MCP sampling capability today; pass `panelists=["host", ...]` explicitly when running under an MCP host that does. `tools/multiaudit.py` + `tools/bugfind.py`.
...
- **OAuth-first extends to direct-API tools** — but the precise contract is async-only and gated. `OAuthFirstProvider` (`providers/oauth_first.py`) wraps every registered provider when `PANEL_OAUTH_FIRST=1` (default). The OAuth-first routing fires when ALL of these gates pass: (1) `PANEL_OAUTH_FIRST != "0"`, (2) the model is in `clink.constants.MODEL_TO_CLI` (gpt-5.5 → codex, gemini-3.1-pro-preview → gemini, claude-opus-4-8 / claude-sonnet-5 → claude), (3) the mapped CLI is on PATH (`shutil.which`), and (4) the call is **async** (`agenerate_content`). When all four pass, the request routes through `execute_tool('clink', ...)` — same canonical dispatch path, same execution-graph child run, same redacted progress events. **Sync `generate_content` is API-only** — routing it through a subprocess would block the calling thread on subprocess I/O. Sync calls for OAuth-eligible models log a WARNING and stamp `oauth_route_skipped=sync_path` on the response metadata so the bypass is observable in cost rollup. **CLI missing from PATH** → default behaviour falls through to direct API with a logger.warning (panel-flagged: previously silent), `cost_tier=api_paid`, and `oauth_route_skipped=cli_not_installed`. Set `PANEL_OAUTH_FIRST_STRICT=1` to ABORT instead of fall through — useful when you want a loud failure rather than a surprise bill. clink itself raises hard (config broken / its own paid-API fallback also failed) → propagate the exception (no silent retry — clink may have already billed once; doubling-up risks two charges per call). Re-entrance guarded by the `_INSIDE_OAUTH_FIRST` ContextVar so clink's internal CLI→API fallback (which calls `execute_tool('chat', ...)` with the same flagship model) doesn't infinite-loop. **Conservative exact-match mapping** — non-flagship variants (`gpt-5.4`, `grok-*`, etc.) bypass the wrapper because they have no CLI route. Note: when calling `chat(model='gpt-5.5')` directly, OAuth-first is what wraps the call — but only if `OPENAI_API_KEY` is also set (the inner provider must register first). No API key → no provider → no OAuth-first wrapping → call fails at provider lookup.
...
- **Panel-mode preamble is prepended to every panel-dispatched prompt.** `tools/panel.py:PanelTool.execute` injects `PANEL_MODE_PREAMBLE` at the top of the user prompt before fan-out. Reaches multiaudit + bugfind + ask_panel + direct panel calls. The preamble forbids `files_required_to_continue` / `files_needed` / `mandatory_instructions` clarification JSON (panel is fire-and-forget; clarification stalls the slot) and forbids running typecheckers / test runners / linters / builds (panelists' job is to read code, not execute it). The preamble is the one signal that overrides `chat_prompt.py`'s clarification protocol for chat-API panelists (grok, OAuth-fallback paths) — without it, those panelists inherit the protocol and stall when they decide they need more files. Idempotent (skips re-injection if the preamble is already present, e.g. when a tool's rubric already embedded equivalent text). Don't remove or weaken — gemini in particular has a strong tendency to use the clarification JSON shape unprompted.
- **Inbox markers are session-scoped via `claude_pid`.** `~/.panel/inbox/` is one directory shared across every Claude Code on the box. Without scoping, the first Stop/UserPromptSubmit hook to fire claims any pending marker — meaning a multiaudit fired from repo A can deliver its verdict into Claude Code session B (different repo). `utils/task_inbox.py:_detect_owner_pid()` captures Panel's PPID at module-load time (= the Claude Code that launched it) and stamps every marker with `claude_pid`. `panel_cli/inbox_drain.py:_drain_pending` peeks each marker's `claude_pid` and only claims those matching its own `os.getppid()`. Markers without `claude_pid` (legacy / orphaned Panel) drain via the unowned fallback so old deployments don't strand. The watch lock is per-session too (`.watch.lock.<pid>`) so each Claude session runs its own poller — without per-session locks, only one session's hook would ever long-poll and the other's markers would go unnoticed mid-watch. Override Panel's owner PID via `PANEL_OWNER_PID` (drain side reads the same env var) for non-stdio launch shapes where direct-child-of-Claude doesn't hold.
- **`systemprompts/clink/default.txt` is the universal CLI-agent floor.** Every clink call (any tool, any mode) sees these rules: (a) read code, don't run typecheckers / test runners / linters / builds — CI runs those, (b) don't return `files_required_to_continue` / `files_needed` JSON — Panel is single-shot for most tools and clarification stalls. Multiaudit + bugfind + ask_panel reinforce these via the panel-mode preamble; clink-direct calls (chat-style consultation) get only this file. Don't move these rules to per-tool prompts — the floor matters for every clink invocation, not just panel mode.
- **`multiaudit` and `bugfind` are SELF-ASYNC; `panel` and `ask_panel` are NOT.** This distinction is load-bearing for the LLM-facing async pattern. multiaudit/bugfind dispatch their panel via `start_task` INTERNALLY and return immediately with a task_id pointing at that inner run — wrapping them in another `start_task` produces a confusing 0s "completed" hook (the outer wrapper finishes in ~90ms while the real debate runs under an inner task_id the caller never sees). `_SELF_ASYNC_TOOLS` in `tools/tasks.py` enforces this: `start_task(tool="multiaudit"|"bugfind", ...)` is rejected with a clear error that echoes the correct shape. `panel` and `ask_panel` are SYNCHRONOUS fan-outs (await the gather of all panelists before returning) and SHOULD be wrapped in start_task — they routinely run for minutes. The handshake instructions spell this out so terse-prompted LLMs default to the right pattern; don't loosen them. Keep `_SELF_ASYNC_TOOLS` in sync with any new tool whose body itself dispatches via start_task.
...
tools/
  ...
  multiaudit.py            Magic-phrase PR audit. Reads git diff, fires
                           ...
                           [codex, gemini, sonnet, opus]. host is
...
| `PANEL_API_TIMEOUT_S` | 1800 | Per-call SDK timeout (bounds thread lifetime). Matches panelist defaults so OAuth-fallback API calls don't get killed by the SDK while the panelist budget still has room. |
...
| `PANEL_OAUTH_FIRST` | 1 | Wrap every direct-API provider in `OAuthFirstProvider` so models with a CLI route (gpt-5.5, gemini-3.1-pro-preview, claude-opus-4-8, claude-sonnet-5) try the free OAuth path first. Set `0` to opt out — every call hits the paid API directly, regardless of CLI auth state. |
| `PANEL_OAUTH_FIRST_STRICT` | unset | When set to `1`, OAuth-first ABORTS instead of falling through to paid API when the mapped CLI is missing from PATH. Default behaviour is fall-through-with-warning (preserves availability). Use strict mode when you'd rather get a loud failure than a surprise bill. |
| `PANEL_INBOX_DIR` | `~/.panel/inbox` | Directory where TaskManager writes completion markers and the drain hook reads them. Override for tests / non-standard layouts. |
| `PANEL_INBOX_DISABLE` | unset | Set to `1` to skip marker writes entirely (e.g. in tests, or for users not running Claude Code's drain hook). |
| `PANEL_OWNER_PID` | unset | Override the Claude Code session PID stamped on each marker (Panel side) AND matched against on drain (hook side). Default behaviour: Panel uses its PPID at module-load time, drain hook uses its `os.getppid()`. Override only when the launch shape breaks the direct-child-of-Claude assumption — e.g. Panel run via a long-lived wrapper, or hook command wrapped in `bash -c`. |
| `PANEL_STOP_WATCH_TIMEOUT_S` | 900 | How long the Stop-hook drain watcher polls before giving up. Set lower for tests. |
| `PANEL_STOP_WATCH_POLL_S` | 1.0 | Poll interval inside the watch loop. |
...
- **Claude CLI** — `claude /login` (Claude subscription). API fallback: configurable via `PANEL_CLAUDE_OAUTH_FALLBACK_MODEL` (default `claude-sonnet-5`) via `ANTHROPIC_API_KEY`.
- **Grok** has no OAuth path — always paid via `XAI_API_KEY`.

When the user names "codex" / "gemini" / "claude" without a specific paid model, prefer `clink` (free, with automatic API fallback). When they name a specific paid string (`gpt-5.5`, `gemini-3.1-pro-preview`, `grok-4.3`, `claude-opus-4-8`, etc.) use `chat`/`consensus`. The MCP handshake instructions encode this routing.
...
The infrastructure is solid: 30 tools, 97 tests passing on the new surfaces, three size gates respect provenance, web viewer is XSS-hardened + lazy-started + streaming-v1. Outstanding for the next session:
