...
- **Internal dispatches bypass the MCP transport size check.** `tools/shared/base_tool.py` tracks dispatch depth via a contextvar; `check_prompt_size` returns None at depth ≥ 2. PAL-generated prompts (multiaudit's diff package, panel's debate-round prompts, clink's OAuth-fallback prompts) never crossed the MCP boundary as raw user input, so the 60K char limit doesn't apply to them. The check still fires at the boundary for genuine user-pasted content.
