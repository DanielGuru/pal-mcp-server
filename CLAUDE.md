...
3. **TaskManager → execution graph migration** — DONE for completed tasks. Lifecycle is persisted to a `tasks` table in the graph DB (see `ExecutionGraph.upsert_task` / `get_task`); `task_result(task_id)` falls back to the DB on memory miss so finished outputs survive PAL restart. In-flight tasks are NOT recovered — their worker thread is gone — but the API surfaces a clear "interrupted by PAL restart" error rather than "unknown task_id" when a non-terminal record is found in the DB.
...
9. **Web viewer auth.** Bound to 127.0.0.1 by default; non-localhost binds now refuse to start without `PAL_WEB_ALLOW_REMOTE=1` (opt-in gate against accidental exposure). Full token auth still TODO when someone actually needs remote — the gate is the safety net.
