...
1. **Streaming v2 — per-token for direct-API providers.** Anthropic now streams unconditionally (`providers/anthropic.py` uses `client.messages.stream` and pumps text-chunk events into the graph). OpenAI / xAI streaming is opt-in via `PAL_OPENAI_STREAM=1` because integration cassette tests freeze the request body hash to the non-streaming shape — flip the env var on for production, re-record cassettes when ready. Still TODO: Gemini (different SDK shape — `client.models.generate_content_stream`) and the OpenAI Responses endpoint (gpt-5.1-codex / o3-pro family — separate streaming API).
