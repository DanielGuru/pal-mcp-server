...
| `PANEL_OAUTH_FIRST` | 1 | Wrap every direct-API provider in `OAuthFirstProvider` so models with a CLI route (gpt-5.5, gemini-3.1-pro-preview, claude-opus-4-8, claude-sonnet-4-6) try the free OAuth path first. Set `0` to opt out — every call hits the paid API directly, regardless of CLI auth state. |
