"""Centralized environment variable access for Panel MCP Server."""
...
def _compute_force_override(values: Mapping[str, str | None]) -> bool:
    raw = (values.get("PANEL_MCP_FORCE_ENV_OVERRIDE") or "false").strip().lower()
    return raw == "true"
...
def env_override_enabled() -> bool:
    """Return True when PANEL_MCP_FORCE_ENV_OVERRIDE is enabled via the .env file."""

    return _FORCE_ENV_OVERRIDE


def get_env(key: str, default: str | None = None) -> str | None:
    """Retrieve environment variables respecting PANEL_MCP_FORCE_ENV_OVERRIDE."""
