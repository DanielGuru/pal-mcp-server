"""
Unit tests for Docker configuration and implementation of Panel MCP Server
...
class TestDockerConfiguration:
    """Tests for Docker configuration of Panel MCP Server"""
    ...
    def test_docker_compose_configuration(self):
        ...
        assert "panel-mcp" in content, "Service panel-mcp must be defined"
...
class TestDockerCommands:
    ...
    def test_docker_build_command(self, mock_run):
        ...
        subprocess.run(
            ["docker", "build", "-t", "panel-mcp-server:latest", str(self.project_root)], capture_output=True, text=True
        )
    ...
    def test_docker_run_command_structure(self, mock_run):
        ...
        assert "panel-mcp-server:latest" in cmd, "Must reference the image"
    ...
    def test_docker_health_check(self, mock_run):
        ...
        subprocess.run(
            ["docker", "run", "--rm", "panel-mcp-server:latest", "python", "/usr/local/bin/healthcheck.py"],
            capture_output=True,
            text=True,
        )
...
class TestMCPIntegration:
    ...
    def test_mcp_configuration_generation(self):
        ...
        pal_docker = expected_config["servers"]["panel-docker"]
...
class TestDockerPerformance:
    ...
    def test_image_size_optimization(self):
        ...
        # result = subprocess.run(['docker', 'images', '--format', '{{.Size}}', 'panel-mcp-server:latest'])
        # Here we simulate
