...
class TestDockerClaudeDesktopIntegration:
    ...
    def test_mcp_config_docker_run_format(self):
        ...
        assert "panel-mcp" in config["mcpServers"]
        assert config["mcpServers"]["panel-mcp"]["command"] == "docker"

        args = config["mcpServers"]["panel-mcp"]["args"]
        assert "run" in args
        assert "--rm" in args
        assert "-i" in args
        ...
        config = {
            "mcpServers": {
                "panel-mcp": {
                    ...
                    "args": ["-f", "/path/to/docker-compose.yml", "run", "--rm", "panel-mcp"],
                }
            }
        }

        # Validate configuration structure
        assert config["mcpServers"]["panel-mcp"]["command"] == "docker-compose"

        args = config["mcpServers"]["panel-mcp"]["args"]
        assert "-f" in args
        assert "run" in args
        assert "--rm" in args
        assert "panel-mcp" in args
    ...
    def test_windows_path_format(self):
            ...
            "mcpServers": {
                "panel-mcp": {
                    ...
                    "args": [
                        ...
                        "-i",
                        "--env-file",
                        "C:/Users/User/panel-mcp-server/.env",
                        "-v",
                        "C:/Users/User/panel-mcp-server/logs:/app/logs",
                        "panel-mcp-server:latest",
                    ],
                }
            }
        ...
        args = windows_config["mcpServers"]["panel-mcp"]["args"]
    ...
    def test_mcp_config_validation(self):
        ...
        # Valid configuration
        valid_config = {
            "mcpServers": {"panel-mcp": {"command": "docker", "args": ["run", "--rm", "-i", "panel-mcp-server:latest"]}}
        }
    ...
    def test_docker_image_reference(self):
        ...
        configs = [
            {"image": "panel-mcp-server:latest"},
            {"image": "panel-mcp-server:v1.0.0"},
            {"image": "registry/panel-mcp-server:latest"},
        ]
    ...
    def temp_mcp_config(self):
        ...
        config = {
            "mcpServers": {
                "panel-mcp": {
                    "command": "docker",
                    "args": ["run", "--rm", "-i", "--env-file", "/tmp/.env", "panel-mcp-server:latest"],
                }
            }
        }
...
class TestDockerMCPErrorHandling:
    ...
    def test_missing_docker_image_handling(self):
        ...
        nonexistent_config = {
            "mcpServers": {"panel-mcp": {"command": "docker", "args": ["run", "--rm", "-i", "nonexistent:latest"]}}
        }

        # Configuration should be valid even if image doesn't exist
        assert "panel-mcp" in nonexistent_config["mcpServers"]
    ...
    def test_invalid_env_file_path(self):
        ...
        config_with_invalid_env = {
            "mcpServers": {
                "panel-mcp": {
                    "command": "docker",
                    "args": ["run", "--rm", "-i", "--env-file", "/nonexistent/.env", "panel-mcp-server:latest"],
                }
            }
        }

        # Configuration structure should still be valid
        args = config_with_invalid_env["mcpServers"]["panel-mcp"]["args"]
        assert "--env-file" in args
    ...
    def test_resource_limit_configurations(self):
        ...
        config_with_limits = {
            "mcpServers": {
                "panel-mcp": {
                    "command": "docker",
                    "args": ["run", "--rm", "-i", "--memory=512m", "--cpus=1.0", "panel-mcp-server:latest"],
                }
            }
        }

        args = config_with_limits["mcpServers"]["panel-mcp"]["args"]
