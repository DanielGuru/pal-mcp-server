...
class MultiauditTool(BaseTool):
    ...
    def get_description(self) -> str:
        return (
            ...
            "multiaudit dispatches the panel via start_task internally and "
            ...
            "debate runs under an inner task_id you'd never see). The server "
            "rejects the double-wrap with a clear error to enforce this. "
            "**FIRE AND FORGET after dispatch.** Hand the user the web viewer "
            "URL from the response, tell them the audit is running, and END "
            "YOUR TURN. Do NOT silently poll task_status / task_result in a "
            "loop — the Stop hook delivers the verdict via system-reminder "
            "automatically when the panel finishes (with the digest inline). "
            "Polling shows the user nothing while it runs and wastes turns. "
            "Only check status if the user explicitly asks. "
