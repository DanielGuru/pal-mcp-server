...
docker build -t panel-mcp-server:latest .
...
docker-compose exec panel-mcp python server.py
```
...
# Stop all panel-mcp containers
docker stop $(docker ps -q --filter "ancestor=panel-mcp-server:latest")
...
```bash
# View logs
docker-compose logs -f panel-mcp

# Check status
docker-compose ps

# Restart service
docker-compose restart panel-mcp

# Stop services
docker-compose down

# Rebuild and update
docker-compose build --no-cache panel-mcp
docker-compose up -d panel-mcp
```
...
- **`panel-mcp-config:/app/conf`** - Configuration persistence (named Docker volume)
- **`/etc/localtime:/etc/localtime:ro`** - Host timezone synchronization (read-only)

### How Persistent Volumes Work

The `panel-mcp` service (used by `panel-docker-compose` and Docker Compose commands) mounts the named volume `panel-mcp-config` persistently. All data placed in `/app/conf` inside the container is preserved between runs thanks to this Docker volume.
...
docker images panel-mcp-server

# Test container interactively
docker run --rm -it --env-file .env panel-mcp-server:latest bash

# Check environment variables
docker run --rm --env-file .env panel-mcp-server:latest env | grep API

# Test with minimal configuration
docker run --rm -i -e GEMINI_API_KEY="test" panel-mcp-server:latest python server.py
```
...
echo '{"jsonrpc": "2.0", "method": "ping"}' | docker run --rm -i --env-file .env panel-mcp-server:latest python server.py
...
docker run --rm --env-file .env panel-mcp-server:latest python -c "import os; print('GEMINI_API_KEY:', bool(os.getenv('GEMINI_API_KEY')))"

# Test API connectivity
docker run --rm --env-file .env panel-mcp-server:latest python /usr/local/bin/healthcheck.py
```
...
docker run --rm -i --memory="512m" --env-file .env panel-mcp-server:latest

# Monitor Docker logs
docker run --rm -i --env-file .env panel-mcp-server:latest 2>&1 | tee docker.log
```
  ...
  "servers": {
    "panel-docker": {
      ...
      "args": [
        ...
        "-i",
        "--env-file",
        "/absolute/path/to/panel-mcp-server/.env",
        "-v",
        "/absolute/path/to/panel-mcp-server/logs:/app/logs",
        "panel-mcp-server:latest"
      ]
    }
  }
  ...
  "servers": {
    "panel-docker": {
      ...
      "args": [
        ...
        "-i",
        "--env-file",
        "C:/Users/YourName/path/to/panel-mcp-server/.env",
        "-v",
        "C:/Users/YourName/path/to/panel-mcp-server/logs:/app/logs",
        "panel-mcp-server:latest"
      ]
    }
  }
...
```json
{
  "servers": {
    "panel-docker": {
      ...
      "args": [
        "-f",
        "/absolute/path/to/panel-mcp-server/docker-compose.yml",
        "run",
        "--rm",
        "panel-mcp"
      ]
...
docker run --rm panel-mcp-server:latest python --version

# Test health check
docker run --rm -e GEMINI_API_KEY="test" panel-mcp-server:latest python /usr/local/bin/healthcheck.py
```
...
echo '{"jsonrpc": "2.0", "method": "initialize", "params": {}}' | \
  docker run --rm -i --env-file .env panel-mcp-server:latest python server.py
```
...
The Panel MCP Server provides these tools when properly configured:
